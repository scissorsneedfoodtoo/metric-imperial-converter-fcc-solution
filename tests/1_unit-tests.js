...
suite('Unit Tests', function() {
  ...
  suite('Function convertHandler.getNum(input)', function() {
    ...
    test('Decimal Input', function(done) {
      ...
      assert.strictEqual(Number(convertHandler.getNum(input)), 1242.663);
      done();
    });

    test('Fractional Input', function(done) {
      const input = '4/2kg';
      assert.strictEqual(Number(convertHandler.getNum(input)), 2);
      done();
    });

    test('Fractional Input w/ Decimal', function(done) {
      const input = '2424.521/921.67lbs';
      assert.strictEqual(Number(convertHandler.getNum(input)), 2.63057);
      done();
    });
    ...
    test('No Numerical Input', function(done) {
      assert.equal(Number(convertHandler.getNum('lbs')), 1);
      assert.equal(Number(convertHandler.getNum('gal')), 1);
      done();
    });
  ...
  suite('Function convertHandler.getUnit(input)', function() {
    ...
    test('Unknown Unit Input', function(done) {
      assert.strictEqual(convertHandler.getUnit('124lbss'), 'invalid unit');
      assert.strictEqual(convertHandler.getUnit('934kms'), 'invalid unit');
      done();
    });
